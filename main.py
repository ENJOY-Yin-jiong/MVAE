...
from core.utils import set_th_config
    ...
    if args.mode.lower() == "train":
        ...
        for epoch in range(config.TRAIN.MAX_EPOCH):
            ...
            for data in dataloader_train:
                ...
                step += 1
                # ------------------------------------------model trainer-----------------------------------------------------------
                batch_x = data['x'].to(device)
                batch_y = data['y'].to(device)
                batch_mask = data['mask'].to(device)


                with torch.cuda.amp.autocast():
                    output, cb_loss, m_loss = model(
                        batch_x,
                        batch_y,
                        batch_mask,
                    )
                    ...
                    #     + config.LOSS.CONSISTENCY * consistency_loss
                    # )
                    # total_loss = config.LOSS.LOCALIZATION * loc_loss + config.LOSS.MATCH * match_loss

                if global_step % 50 == 0:
                    logger.info(
                        "epoch: {}, step: {}/{}, lr: {:.6f},  total: {:.4f}, cb_loss: {:.4f}, m_loss: {:.4f}".format(
                            epoch + 1,
                            step,
                            step_epoch,
                ...
                writer.add_scalar("Loss_train/codebook", cb_loss, global_step)
                writer.add_scalar("Loss_train/music", m_loss, global_step)
                writer.add_scalar(
                    "learning_rate",
                    optimizer.state_dict()["param_groups"][0]["lr"],
                    global_step,
